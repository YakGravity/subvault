...
.PHONY: build
build:
	go build -ldflags "$(LDFLAGS)" -o subvault cmd/server/main.go
...
.PHONY: build-all
build-all:
	GOOS=darwin GOARCH=amd64 go build -ldflags "$(LDFLAGS)" -o dist/subvault-darwin-amd64 cmd/server/main.go
	GOOS=darwin GOARCH=arm64 go build -ldflags "$(LDFLAGS)" -o dist/subvault-darwin-arm64 cmd/server/main.go
	GOOS=linux GOARCH=amd64 go build -ldflags "$(LDFLAGS)" -o dist/subvault-linux-amd64 cmd/server/main.go
